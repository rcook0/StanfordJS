...
public class SJSConstKeyword extends PrefixOperator {
   public Expression prefixAction(Parser p) {
      ...
      if (jsp.getTokenType(variable) != 1) {
         ...
         Expression exp = null;
         if (token.equals("=")) {
            exp = jsp.createCompound2(jsp.getOperator(token), jsp.createIdentifier(variable), jsp.readE(0));
         } else {
            if (!token.equals(";")) {
            ...
            jsp.saveToken(token);
            exp = jsp.createCompound2(jsp.getOperator(token), jsp.createIdentifier(variable), jsp.createConstant(Value.UNDEFINED));
         }
